...
- Registration
    ...
    - Components are provided a template instead of mounting with the `el` option
        - Only the root Vue instance (defined using `new Vue`) will include an `el` to mount to
...
- It is a very bad idea to mutate parent state from a child component
    ...
    2. Makes the parent state much harder to reason about when looking at it alone, because its state may be modified by any child. Ideally, only a component itself should be allowed to modify its own state
- All Vue instances implement a custom event interface that facilitates communication within a component tree
...
- This won't pass any data to the component because components have isolated scopes of their own. In order to pass the iterated data to the component, use props too
